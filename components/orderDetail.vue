<template>
  <div class="flex flex-col space-y-4">
    <div class="flex space-x-4" v-for="(item, index) in data.products" :key="index">
      <img :src="item.image_url" class="w-28 h-28 transition-transform duration-500 ease-in-out transform hover:scale-105 cursor-pointer" @click="navigateToProduct(item.product_id)" />
      <div class="flex flex-col w-52"> 
        <h3 class="text-lg text-wrap font-medium">{{ item.product_name }}</h3>
        <h4 class="w-52 text-sm truncate">{{ item.product_description }}</h4>
      </div>
      
      <h4 class="p-3">{{ item.quantity }}</h4>
      <h4 class="py-3 font-medium">{{ Intl.NumberFormat('vi-VN', {
          style: 'currency', currency: 'VND'
      }).format(item.product_price * item.quantity) }}</h4>
    </div>
  </div>
</template>
